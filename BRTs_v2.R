...
for(file in files){
  ...
  df <- data %>%
    select(pa, depth, dist2col, dist2coast, slope, x, y)
  ...
  rec <- recipe(pa ~ depth + dist2col + dist2coast + slope, data = df) %>%
    step_downsample(pa)
  ...
  df2 <- df2 %>%
    select(dive_intensity, depth, slope, dist2coast, dist2col, x, y)
  ...
  # define formula for modelling
  rec <- recipe(dive_intensity ~ dist2col + dist2coast + depth + slope, data = df2) 
